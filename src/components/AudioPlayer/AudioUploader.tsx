  ...
  const processFiles = (files: File[]) => {
    ...
    if (audioFiles.length < files.length) {
      toast({
        ...
        description: `${files.length - audioFiles.length} non-audio files were ignored`,
        variant: "default"
      });
    }
    ...
    toast({
      ...
      description: `${audioFiles.length} audio ${audioFiles.length === 1 ? 'file' : 'files'} ready to play`,
      variant: "default"
    });
    ...
    <div className="space-y-4">
      ...
      <div 
        className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${
          isDragging ? 'border-audiobook-purple bg-audiobook-lightPurple/10' : 'border-gray-300'
        }`}
